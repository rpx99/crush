...
func (m *UI) computeSystemMessages() []chat.MessageItem {
	...
	for _, kind := range m.activeSystemMessageKinds() {
		...
		case chat.SystemMessageSysadmin:
			items = append(items, m.newSysadminWarning())
		}
	}
	return items
}
...
func (m *UI) activeSystemMessageKinds() []chat.SystemMessageKind {
	...
	// Sysadmin mode warning. Read the cached mode rather than
	// the workspace: this runs on every refresh, and the toggle path writes
	// the new value through the cache precisely to avoid a re-probe.
	if m.permModeCached() == permission.PermissionModeSysadmin {
		kinds = append(kinds, chat.SystemMessageSysadmin)
	}
...
// initialSuppressedSystemMessages returns the dismissal set a fresh UI starts
// with. The context warning starts dismissed so launching Crush on a small
// model does not greet the user with a nag about a choice they already made;
// it surfaces on the next model change or new session, where it is news.
func initialSuppressedSystemMessages() map[chat.SystemMessageKind]bool {
	return map[chat.SystemMessageKind]bool{
		chat.SystemMessageContextWarning: true,
	}
}
...
// newSysadminWarning builds the sysadmin mode advisory. Every run is
// colored explicitly so the body survives width reflow.
func (m *UI) newSysadminWarning() chat.MessageItem {
	sty := m.com.Styles
	base := sty.Messages.SystemBody.Render
	accent := sty.Messages.SystemAccent.Render
	body := base("Sysadmin mode is on. Crush will run every command automatically, including destructive ones, without asking. Only use this when you fully trust the task at hand.") +
		"\n\n" +
		base("Toggle it off any time from the command palette (") +
		accent(keybindLabel(m.keyMap.Commands)) +
		base(").")
	return chat.NewSystemMessageItem(sty, chat.SystemMessageSysadmin, "Sysadmin Mode", body)
}
