...
func TestComputeSystemMessages(t *testing.T) {
	...
	t.Run("sysadmin warns", func(t *testing.T) {
		t.Parallel()
		ui := newSystemMessagesUI(t, 400_000, permission.PermissionModeSysadmin)
		require.Equal(t, []chat.SystemMessageKind{chat.SystemMessageSysadmin}, kinds(ui.computeSystemMessages()))
	})
...
func TestSystemMessageSuppression(t *testing.T) {
	...
	t.Run("cold start stays quiet about the context window", func(t *testing.T) {
		t.Parallel()
		ui := newSystemMessagesUI(t, 5000, permission.PermissionModeSysadmin)
		ui.suppressedSystemMessages = initialSuppressedSystemMessages()

		require.Equal(t,
			[]chat.SystemMessageKind{chat.SystemMessageSysadmin},
			kindsOf(ui.computeSystemMessages()),
			"launching on a small model does not nag, but sysadmin still does",
		)

		// A model switch is news, so the advisory surfaces.
		ui.retriggerSystemMessage(chat.SystemMessageContextWarning)
		require.Len(t, ui.computeSystemMessages(), 2, "model switch surfaces the warning")
	})

	t.Run("new session surfaces the context warning", func(t *testing.T) {
		t.Parallel()
		ui := newSystemMessagesUI(t, 5000, permission.PermissionModeNormal)
		ui.suppressedSystemMessages = initialSuppressedSystemMessages()
		require.Empty(t, ui.computeSystemMessages())
		...
		ui.resetSystemMessageSuppression()
		require.Len(t, ui.computeSystemMessages(), 1, "a fresh session warns")
	})
}

// kindsOf extracts the advisory kinds from a set of rendered items.
func kindsOf(items []chat.MessageItem) []chat.SystemMessageKind {
...
func newSystemMessagesUI(t *testing.T, contextWindow int64, mode permission.PermissionMode) *UI {
	...
	// selectedLargeModel reads the memoized probe rather than asking the
	// workspace, so seed that cache the way a ready agent would.
	model := workspace.AgentModel{
		CatwalkCfg: catwalk.Model{Name: "Test Model", ContextWindow: contextWindow},
		ModelCfg:   config.SelectedModel{Provider: "test"},
	}
	...
	// The advisory reads the memoized permission mode, which toggles write
	// through, so seed it rather than the workspace.
